...
const createNodeSwapper = (context: UtilRuleContext<string, RuleOptions>) => {
  ...
  const getRangeWithIndent = (node: TSESTree.Comment) => [
   ...
   */
  const getLineRange = (node: TSESTree.Comment): TSESTree.Range => {
  ...
  const getIndentText = (node: TSESTree.Node) =>
    sourceCode.text.slice(...getIndentRange(node));

  const getNodePunctuator = (node: TSESTree.Node) => {
  ...
  return (
    ...
    [currentNode, replaceNode].reduce<RuleFix[]>((acc, node) => {
      ...
      const isLastReplacingLast =
        ...
        nodePositions.get(node)?.final ===
          nodePositions.get(otherNode)?.initial;
